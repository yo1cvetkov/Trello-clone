...
// The core functionality for drag and drop
export default function setup(onDragComplete) {
  // Look for mousedown on any draggable element
  addGlobalEventListener("mousedown", "[data-draggable]", (e) => {
    // Get the exact item we are dragging

    const selectedItem = e.target;
    const itemClone = selectedItem.cloneNode(true);
    const ghost = selectedItem.cloneNode(); // we dont care about content inside so we dont need true
...
function setupDragEvents(selectedItem, itemClone, ghost, offset, onDragComplete) {
  ...
  // When we move mouse as long as holding down the mouse click print moved
  document.addEventListener("mousemove", mouseMoveFunction);
  // When we let the mouse click up remove the listener for mouse move
...
// Create clone of the dragged element in order to change its position
